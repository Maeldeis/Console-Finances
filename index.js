var finances = [
  ...
  // Assuming your dataset is an array of objects with Date and ProfitLoss fields
const dataset = [
  ...
  for (let i = 0; i < dataset.length; i++) {
    ...
    const currentDate = dataset[i].Date;
    const currentProfitLoss = dataset[i].ProfitLoss;
  
    // Calculating total months and total profit/loss
    totalMonths++;
    totalProfitLoss += currentProfitLoss;
  
    // Calculating change in profit/loss (except for the first month)
    if (i > 0) {
      const change = currentProfitLoss - dataset[i - 1].ProfitLoss;
      totalChange += change;
  ...
  // Displaying or logging the results
